...
## 2026-08-19 — Session 18 (PharmCAT live-comparison runbook)

**What happened:** the user confirmed `nextflow run main.nf ...` finally succeeded end to end on their WSL machine (Nextflow 26.04.3) and committed. Asked what to do next; offered three options (live PharmCAT comparison, NUDT15, or v2 scope planning). User picked live PharmCAT comparison and asked what further steps to plan.

**What was delivered:** `docs/PHARMCAT_LIVE_COMPARISON_RUNBOOK.md` -- a runnable runbook for the one remaining Phase 7 checklist item that genuinely cannot be done from inside this sandbox (no Java 17+, no bcftools/htslib, PharmCAT's own installer/release hosts blocked by the sandbox's network allowlist -- same category of restriction as Nextflow's own binary download, Sessions 14-17). Uses this repo's own real GeT-RM fixtures (one curated sample per gene: TPMT/NA12753, DPYD/HG00118, SLCO1B1/NA10847, CYP2C19/GM17203) so a first run needs no external data beyond a GRCh38 reference FASTA. States plainly, not glossed over: this repo's VCF fixtures are deliberately minimal (2-5 positions per file), and whether PharmCAT's VCF Preprocessor tolerates a file that sparse is untested -- genuinely unknown until the user runs it.

**Verification done this session:** the runbook's own commands were reviewed against PharmCAT's public documentation (conda/bioconda install path, `pharmcat_pipeline` CLI usage) but not executed -- the same category of "reviewed carefully, not run" limitation as `main.nf` before Session 14's first real test. Also gave a prioritized roadmap recommendation in prose (hold live PharmCAT results as top priority once run; then NUDT15; then CYP2C19/VKORC1; then container/Conda profile or CYP2D6 feasibility writeup as lower priority) -- not written into a doc, just stated in chat.

**Not done yet:** the actual live PharmCAT run itself -- needs the user's own machine. No HANDOFF entry was written for this delivery in the session it happened; backfilled here so the numbering stays contiguous with what follows.

**Next session should:** wait for the user's PharmCAT results (or move on if they pick a different next step, which is what happened -- see Session 19).

---

## 2026-08-19 — Session 19 (NUDT15: research, implementation, compound TPMT+NUDT15 Tier 2 recommendation)

**What happened:** the user said "PharmCAT committed. Let's do NUDT15 next" -- explicit direction, matching the prioritized roadmap from Session 18.

**Research done first, before any code (this project's own standing discipline):**
- Confirmed via direct dbSNP/ClinVar lookups that PharmVar's real NUDT15 catalog has ~20 star alleles, and identified a defensible, real-reason-driven v1 scope: implement only c.415C>T (p.Arg139Cys, "R139C", rs116855232, GRCh38 chr13:48,045,719 C>T), which distinguishes `*1` from PharmVar's 2025-consolidated no-function allele `*3` (folding the former `*2`/`*3` into suballeles `*3.001`/`*3.002`, both sharing R139C).
- Found and documented a real, citable discrepancy: the GAGTCG-insertion variant that further distinguishes `*3.002`/`*6` is cited by both CPIC's 2018 guideline and PharmVar's own nomenclature paper as rs869320766 -- but that rsID could not be resolved via NCBI's variation API or myvariant.info. ClinVar cross-references the genomically equivalent variant (`NM_018283.4(NUDT15):c.38GAGTCG[4]`) to a *different* rsID, rs746071566, at GRCh38 chr13:48,037,782-48,037,783. This, plus the lack of a reference FASTA in this sandbox for correctly left-aligning a tandem-repeat-expansion indel, is the real, stated reason the insertion variant is out of scope for v1 -- not a shortcut taken without justification.
- Read CPIC's real 2025/2026 TPMT/NUDT15 thiopurine guideline update in full (PMID 41618934, DOI 10.1002/cpt.70209) directly from the primary PDF (fetched via `web_fetch`, too large to return inline at 69,462 characters/1,273 lines -- read via the tool's own saved-to-file fallback). Confirmed the guideline's real Table 1 (phenotype assignment, both genes) and Table 2 (mercaptopurine dosing, both genes jointly) content directly, including the exact quoted dosing text for all four real rows (standard dose; 30-80% reduction; 10-fold reduction/alternative agent; and the new "compound intermediate metabolizer" 20-50% reduction). Confirmed the joint guideline ID (`PA166104933`) is the SAME one this project already fetches for TPMT's existing single-gene azathioprine table.

**Implemented:**
- `pgx_interpreter/genes/nudt15.py` -- one locus, two alleles, direct genotype-to-diplotype mapping (no phasing needed at all, a genuinely different and simpler structural shape than every other gene module in this project). `tests/test_nudt15.py`, 10 tests, all passing first run; 6 new VCF fixtures under `tests/fixtures/nudt15/`.
- `pgx_interpreter/evidence.py`: added `recommend_compound_thiopurine(tpmt_result, nudt15_result, ...)` -- the first Tier 2 recommendation in this project keyed on two genes' `PGxResult`s at once, since CPIC's compound table has no "TPMT known, NUDT15 unknown" row. Implements Table 2 (mercaptopurine) only for v1 -- Table 3 (thioguanine) and Table 4 (azathioprine) are real, documented, out-of-scope gaps (same scoping pattern as CYP2C19's ACS/PCI-column-only clopidogrel scope). The existing single-gene TPMT+azathioprine table (`recommend()`, `_TPMT_RECOMMENDATIONS`) is left completely unchanged -- additive, not a replacement. 11 new tests in `tests/test_evidence.py`, all passing first run, reusing the already-cached `PA166104933.json` fixture (no new network fetch needed).
- `pgx_interpreter/cli.py`: added `NUDT15`/`call_nudt15` to `GENE_CALLERS`; `run_report()` now special-cases the recommendation step -- when both TPMT and NUDT15 are requested, calls `recommend_compound_thiopurine()` once for the pair and attaches the same recommendation to both (superseding TPMT's single-gene table for that report); otherwise every gene goes through the ordinary per-gene `recommend()` path unchanged. New `_recommend_compound_or_warn()` helper mirrors `_recommend_or_warn()`'s offline-friendly-but-not-guessing policy. 2 new CLI-level subprocess tests added; the existing four-gene "normal, with recommendations" test was updated to five genes and to assert the compound path's `mercaptopurine` recommendation on both TPMT and NUDT15.
- `pgx_interpreter/report.py`: added a `NUDT15` entry to `_GENE_LIMITATIONS` (section 9), including the rs869320766/rs746071566 discrepancy and the "needs a TPMT result to get a Tier 2 recommendation" limitation.
- `assets/example_sample_all_normal.vcf` and `nextflow.config`'s default `params.genes`: both updated to include NUDT15, so the existing Nextflow demo now exercises the compound recommendation path too.
- `docs/GENE_SCOPE.md`: new NUDT15 section (mirroring every other gene's section structure) plus updates to the TPMT section noting NUDT15 is no longer out of scope and the compound path now supersedes the single-gene azathioprine table when both genes are requested. `README.md`: status line, gene count, and repository-structure block all updated.

**Verification done this session:** full test suite, both runners -- 196/196 passing (up from 176 before this session: 10 new NUDT15 tests, 11 new compound-recommendation tests, 2 new CLI tests, 1 existing CLI test updated in place). Manually ran the actual CLI end to end against the updated `assets/example_sample_all_normal.vcf` (not just the test suite) and inspected the real JSON output directly: all five genes report `*1/*1`/Normal, and TPMT/NUDT15 both correctly receive the identical mercaptopurine compound recommendation while DPYD/SLCO1B1/CYP2C19 keep their own existing single-gene recommendations unaffected.

**Not done yet (deliberately deferred, stated plainly):**
- **No GeT-RM validation for NUDT15 yet** -- unlike every other v1 gene, this session did not source real reference-material samples for NUDT15. A real, open item for a future session, not an oversight (see `docs/GENE_SCOPE.md`'s NUDT15 section, last line).
- **The GAGTCG-insertion allele (`*3.002`/`*6`) remains out of scope**, for the real reasons above (no reference FASTA, unresolved rsID discrepancy) -- documented in `pgx_interpreter/genes/nudt15.py`'s module docstring and `docs/GENE_SCOPE.md`.
- **Thioguanine and azathioprine compound dosing tables (CPIC Tables 3 and 4) are not implemented** -- only the mercaptopurine table (Table 2). A real, documented scope decision, not silently dropped.
...
## 2026-08-20 — Session 23 (CYP2D6 feasibility writeup)

**What happened:** the user picked "CYP2D6 feasibility writeup" from an `AskUserQuestion` menu of next-step options -- v2 scope planning, not new gene/drug work. CYP2D6 has been listed as out-of-scope since Phase 0 with a one-line reason ("specialist structural-variant-aware calling"); this session worked that reason through in real technical detail against primary sources, rather than adding code.

**Research:** fetched PharmVar's own CYP2D6 structural-variation reference document (`Variation_CYP2D6.pdf`, v1.6, 2019-11-06) directly from pharmvar.org -- real, detailed, quotable primary-source content on the CYP2D6/CYP2D7/CYP2D8 locus structure (REP6/REP7/REPdel/REPdup regions, the 1.6kb spacer distinguishing CYP2D7's downstream region), the `*5` whole-gene deletion, the duplication/multiplication allele family (`*1xN` through `*45x2`, each with its own literature PMID trail), CYP2D6→CYP2D7 hybrids (`*10.003`, `*61`, `*63`, `*68`, `*4.013`), CYP2D7→CYP2D6 hybrids (consolidated as `*13`), and tandem arrangements (`*36+*10`, `*13+*68+*4`, etc.) -- plus PharmVar's own stated caveat that there is no textual standard for reporting CYP2D6 CNV diplotypes across labs. Also fetched/read Caudle KE et al. 2020 (*Clin Transl Sci*, DOI 10.1111/cts.12692, the current CPIC/DPWG Activity Score consensus) and Nassar et al. 2021 (*npj Genomic Medicine*, DOI 10.1038/s41525-020-0135-2), a systematic head-to-head comparison of three specialist CYP2D6 callers (Astrolabe, Aldy, Stargazer) against 75 real GeT-RM WGS samples -- real published concordance numbers (88-96% haplotype concordance even on the easiest, SNV-only test cases; worse for structural variants; PharmCAT itself delegates CYP2D6 calling to Astrolabe rather than implementing its own caller), plus Cyrius's (Twist GP et al. 2021, DOI 10.1038/s41397-020-00205-5) current best-published 99.3% concordance, still short of 100% on an enrichment-biased validation set.

**What was found, argued directly from that research rather than assumed:** CYP2D6 breaks this project's core Layer 1 architectural assumption (a fixed small set of VCF-genotyped SNV/indel positions) at the locus level, not just the allele-catalog level -- a large share of its clinically important alleles are copy-number and structural-rearrangement events with no representation in this project's `ObservedVariant` model, and even short-read alignment itself is unreliable this close to CYP2D7 before any of this project's own code runs. The specialist tools purpose-built for exactly this problem (Aldy, Stargazer, Astrolabe, Cyrius), using full aligned WGS data this project's VCF-only input never has access to, still disagree with each other and with ground truth at a measurable, published rate -- this is close to the current edge of the field, not a gap specific to this project's own implementation effort.

**Delivered:** `docs/CYP2D6_FEASIBILITY.md`, a full technical writeup: the locus-homology problem, PharmVar's structural-variant catalog quoted directly, the specialist-tool concordance comparison, the CPIC/DPWG Activity Score phenotype model (noting real precedent already exists in this project via `genes/dpyd.py`'s own activity-score summation, so that part of a future implementation would not be wholly novel), and a concrete list of what a real v2 implementation would require (alignment-level input, a copy-number layer, a structural/breakpoint layer, a new `ObservedVariant`-equivalent representation, a CYP2D6-specific diplotype assembler, and the Activity Score phenotype layer) -- with an explicit recommendation, if ever pursued, to wrap an existing specialist tool (most plausibly Cyrius) rather than reimplement calling logic multiple specialist research teams have spent years on, mirroring PharmCAT's own precedent of delegating to Astrolabe. `README.md`'s "Explicitly out of scope" line and Documentation list updated to point to it.

**Verification done this session:** no code changed, so the full test suite was re-run purely to confirm no regression from doc edits -- 231/231 passing, both runners, unchanged from Session 22. Every citation in the new document was fetched directly from a primary or peer-reviewed source this session (PharmVar's own PDF, or a paper's own abstract/results text), not reconstructed from memory -- consistent with this project's "never fabricate citations" discipline.

**Not done yet (deliberately deferred, not an oversight):** this is a feasibility assessment only -- no CYP2D6 code, no licensing evaluation of Cyrius/Aldy/Stargazer/Astrolabe for actual integration (a real next step only if the "wrap a specialist tool" path is ever pursued), no effort estimate (depends on a build-vs-wrap decision the document frames but does not make). The PharmCAT live comparison from Session 18 is still pending the user's own run. The container/Conda profile for `main.nf` remains the other open v2-scope item from Session 21's roadmap.

**Next session should:**
1. Sync this batch, review the diff, commit, push.
2. Natural next steps: container/Conda profile for `main.nf` (the other v2-scope item); another CYP2C19 drug pairing if a real third guideline exists; Architecture Review 2 (5 genes, multi-drug pairings, and compound recommendations all now exist -- a natural point to review what's turned out universal vs. gene-specific since Architecture Review 1, mirroring that document's own format).

---

## 2026-08-20 — Session 24 (Architecture Review 2)

**What happened:** the user picked "Architecture Review 2" from an `AskUserQuestion` menu of next-step options -- a milestone checkpoint, not new gene/drug work, mirroring Architecture Review 1's own format and the plan's original discipline of pausing feature development at a review point rather than carrying momentum straight through.

**Method:** every claim in the review was checked directly against the current code (`grep`, direct file reads), the same discipline AR1 modeled and this project's own citation/verification standard requires -- nothing was written from memory of what earlier `HANDOFF.md` entries said was intended. Confirmed directly: `genes/_shared.py`'s zygosity helpers are consumed identically by all five gene modules now (CYP2C19 and NUDT15 added since AR1, both with zero divergence); `models.py` has had zero schema changes since Phase 6's `interpretation_notes` field despite three new architectural situations since (a fourth and fifth gene, CYP2C19's second drug pairing, and the compound two-gene TPMT+NUDT15 recommendation); `report.py` genuinely has no gene-uniqueness assumption anywhere in its code (not just its docstring's claim) -- confirmed via `grep` returning zero matches and direct reading of `build_report()`'s plain-tuple `results` storage; `Confidence.UNRESOLVED` and `ObservedVariant.genotype_quality` are still completely unused across all five real gene modules, unchanged from AR1's finding two phases ago; `schema.validate()` is now exercised in one test (`tests/test_evidence.py::test_populated_recommendation_validates_against_schema`, added since AR1) but AR1's actual recommendation -- wiring it into the real `report.py`/`cli.py` pipeline -- was never carried out, confirmed via `grep` finding zero calls in either file.

**What was found, argued directly from that evidence rather than assumed:** CYP2C19 answers AR1 §6's own closing question -- it needed neither TPMT/SLCO1B1's shared two-linked-variant dosage table nor DPYD's four-independent-loci-decline model, confirming AR1's restraint (not extracting that table after only two examples) was the right call. NUDT15 is a fourth, simplest-yet calling shape (single locus, no phasing question at all) and required zero special-case logic, a useful negative data point AR1 didn't have. The compound TPMT+NUDT15 tables introduced a genuinely new category of special-case logic AR1 had no vocabulary for -- gene-*pair*-specific rather than gene-specific -- accommodated by attaching the identical `RecommendationResult` object to two separate `PGxResult`s rather than any schema change. `evidence.py`'s `_compound_category()` abstraction was extracted using the identical "wait for two-plus real data points before generalizing" discipline AR1 recommended for the zygosity helpers -- applied a second time, in a different part of the codebase, by a different session, without being told to.

**Delivered:** `docs/ARCHITECTURE_REVIEW_V02.md`, following AR1's exact six-question format. Two concrete, low-risk action items recorded (not fixed, matching AR1's own restraint of recording rather than immediately acting): `schema.validate()` is genuinely overdue to be wired into `report.py`'s real pipeline (AR1 flagged this two phases ago and it still hasn't happened); two structurally different CLI extension patterns now exist for "a gene gets a second Tier 2 option" (`--cyp2c19-voriconazole`'s additive pattern vs. `--thiopurine-drug`'s selector pattern), each correct for its own documented reason, but a third case would need to explicitly re-derive which fits rather than there being one obvious shared pattern. `README.md`'s Documentation list updated to point to the new review.

**Verification done this session:** no code changed, so the full test suite was re-run purely to confirm no regression -- 231/231 passing, both runners, unchanged from Session 23. Every claim in the new document was checked directly against the current code this session (grep output, direct file reads quoted above), not reconstructed from memory.

**Not done yet (deliberately deferred, not an oversight):** the two action items this review recorded (`schema.validate()` wiring; the CLI-extension-pattern question) were deliberately left unfixed -- this session's scope was the review itself, matching AR1's own discipline of recording a recommendation and revisiting it once there's a specific reason to act, not doing the fix inline with the review. The container/Conda profile for `main.nf` and the PharmCAT live comparison remain open from prior sessions.

**Next session should:**
1. Sync this batch, review the diff, commit, push.
2. Natural next steps: wire `schema.validate()` into `report.py`'s real pipeline (small, low-risk, now flagged twice); container/Conda profile for `main.nf`; another CYP2C19 drug pairing if a real third guideline exists.

---

## 2026-08-20 — Session 25 (schema.validate() wiring; closing session)

**What happened:** the user asked directly what to do next and how much further the project was worth taking. Given directly, not softened: after five gene modules across three genuinely different calling shapes, two Tier 2 evidence extension patterns, a five-format report layer, CLI/Nextflow orchestration, real GeT-RM validation for all five genes, and two rounds of architecture review that Architecture Review 2 itself confirmed have produced zero schema churn since Phase 6, the feature-work portion of this project has reached a real plateau -- a third CYP2C19 drug pairing or a sixth same-shaped gene would mostly repeat already-demonstrated patterns rather than teach the architecture anything new, and the container/Conda profile would likely hit the same sandbox network-allowlist wall that already blocked live Nextflow and PharmCAT verification (a pattern already on record twice, not needing a third instance). The one item worth still doing was the `schema.validate()` wiring both architecture reviews flagged as a real, low-risk, already-diagnosed gap. The user agreed and asked for exactly that fix, framed as closing out the project rather than opening new scope.

**Implemented:** `pgx_interpreter/report.py`'s `build_report()` now calls `schema.validate()` once per `PGxResult`, at the single point every result already passes through regardless of which gene produced it or how many of the five output formats get rendered afterward -- the exact wiring point both Architecture Review 1 and Architecture Review 2 identified, chosen specifically over validating inside `_gene_section()` (called up to five times per result, once per renderer, which would mean redundant repeated validation of the same object). A validation failure raises `ValueError` naming the gene, sample_id, and every specific problem found, matching `build_report()`'s own existing "fail loudly with specifics" convention (the same pattern its sample-id-mismatch check already used). Module docstring extended with a new section explaining the wiring point and reasoning. Two new tests in `tests/test_report.py`: one confirms a real four-gene result set (TPMT/DPYD/SLCO1B1/CYP2C19, four distinct calling shapes) validates cleanly; the other proves the raise path actually fires, using a minimal duck-typed stand-in object with a deliberately broken `to_dict()` rather than a real gene module, since no real gene module can currently produce invalid output (every required field is a real dataclass field, every enum-typed field is a constrained `Enum`) -- the check exists specifically to catch *future* schema/model drift, which a real fixture can't manufacture on demand.

**Verification done this session:** full test suite, both runners -- 233/233 passing (up from 231, the two new tests), zero regressions elsewhere. A real CLI smoke test (`pgx-interpreter report` against the all-normal demo VCF, all five genes, all four stdlib-only formats) confirmed the real pipeline still succeeds end to end with validation now active on every request, not just in tests. Packaged and verified via a fresh zip extraction + full test re-run, the same discipline as every prior delivery. `docs/ARCHITECTURE_REVIEW_V02.md` got a same-day addendum recording the fix, mirroring Architecture Review 1's own addendum pattern for its own §6 recommendation.

**Not done yet (deliberately deferred, not an oversight, and per this session's own framing not necessarily to be picked up next):** the CLI-extension-pattern question (additive `--cyp2c19-voriconazole` vs. selector `--thiopurine-drug`) from Architecture Review 2 §6 remains correctly unresolved, left for whenever a real third Tier-2-option case arrives rather than decided speculatively. The container/Conda profile for `main.nf` and the PharmCAT live comparison remain open, both blocked on real network access this sandbox doesn't have, not on further design work.

**Next session should:** sync this batch, review the diff, commit, push. This is intended as a closing session for the current phase of work -- the natural next trigger for picking this project back up is a real new requirement (a new gene actually needed for a real use case, a v2 CYP2D6 attempt with real WGS/BAM access, or the container/PharmCAT verification steps run on a machine with real network access), not a scheduled next phase.
