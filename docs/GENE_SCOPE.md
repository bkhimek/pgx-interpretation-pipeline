...
**Tier 2 (drug recommendation, Phase 5; superseded by the compound path when NUDT15 is also requested, see below):** azathioprine, via `pgx_interpreter/evidence.py`, guideline `PA166104933`. Uses CPIC's **single-gene TPMT table** (2018 Update, as reproduced in NCBI Bookshelf NBK100661 Table 2). Normal Metabolizer → normal starting dose; Intermediate → 30-80% of normal dose; Poor → alternative agent or a drastically reduced (10-fold) dose. All three classified "Strong" by CPIC. Only attached when TPMT's phenotype call is `Confidence.SUPPORTED`; ambiguous/insufficient-data/unsupported-allele results are never given a drug recommendation. **Now that NUDT15 is implemented (see below), a report requesting both genes together uses the newer, more specific joint TPMT+NUDT15 mercaptopurine table instead** (`evidence.recommend_compound_thiopurine()`) — this single-gene azathioprine table remains exactly as it was and is still used whenever NUDT15 isn't also requested for the same sample.
...
**A second Tier 2 drug pairing, added in a later session:** voriconazole, guideline `PA166161537` — CPIC's 2016 guideline (Moriyama et al. 2017, PMID 27981572), Table 1 ("adult patients"), fetched directly from ClinPGx's live API, 2026-08-20. This is this project's first gene with more than one Tier 2 pairing: CYP2C19 both *activates* clopidogrel (a prodrug) and *clears* voriconazole directly, so the same phenotype can imply opposite urgency depending on which drug is in play. `evidence.recommend()` gained a `drug` parameter for exactly this (default `None` = clopidogrel, unchanged from before; `drug="voriconazole"` selects the new table) — see `evidence.py`'s module docstring for the full rationale and `cli.py`'s `--cyp2c19-voriconazole` flag, which adds a second, independent CYP2C19 report section rather than changing the existing one. **Real, documented scoping decision, the same pattern as clopidogrel's Table 1/Table 2 split:** the 2016 guideline also publishes a parallel pediatric table (Table 2, different text for rapid/poor metabolizers); this module implements only the adult table (Table 1), since this project's schema has no patient-age field to route to the pediatric one correctly. Ultrarapid/Rapid/Poor Metabolizer → choose an alternative agent not dependent on CYP2C19 metabolism (isavuconazole, liposomal amphotericin B, or posaconazole); Normal/Intermediate Metabolizer → standard of care dosing (Normal is "Strong", Intermediate is "Moderate" — same action, different CPIC-stated confidence, since the Intermediate tier's diplotype examples include the provisionally-classified `*2/*17` genotype).

**Validated against real reference material:** 8 real GeT-RM (CDC) consensus-genotype samples, all exact matches, including one (GM17203, `*2/*17`) that independently confirms this module's central architectural claim — that double-heterozygosity at the `*2`/`*17` loci resolves directly to a compound diplotype rather than requiring phasing — against a real laboratory consensus call, not just a synthetic fixture. See `docs/VALIDATION.md` §3. (This validation predates the voriconazole pairing above; the same 8 diplotype calls apply to both drugs, since Tier 2 recommendation logic doesn't affect Tier 1 allele/phenotype calling.)

## NUDT15 ("let's do NUDT15 next" session)

**Model:** diplotype lookup, like TPMT — but with the narrowest scope of any v1 gene: **one locus, two alleles**. NUDT15 hydrolyzes thiopurine active metabolites; loss of function causes the same downstream toxicity risk as TPMT loss of function, via a different step in the same pathway, which is why CPIC's thiopurine guideline has always dosed by the *joint* TPMT+NUDT15 phenotype (see Tier 2 below — the first two-gene-input recommendation in this project).

**Alleles recognized:** `*1` (reference, normal function), `*3` (consolidated no-function allele — see the scope decision below). One defining SNV, no phasing to resolve (a genuinely different structural shape from TPMT/SLCO1B1's two-linked-variant models — see `pgx_interpreter/genes/nudt15.py`'s module docstring).

**A real, documented scope decision — narrower than every other v1 gene:** PharmVar's real NUDT15 catalog has ~20 star alleles. This module implements only c.415C>T (p.Arg139Cys, "R139C"), which distinguishes `*1` from the no-function allele PharmVar's 2025 nomenclature update now calls `*3` (a consolidation of the former `*2` and `*3`, both sharing this exact substitution — the former `*2` additionally carries a GAGTCG-hexanucleotide repeat expansion in cis, now suballele `*3.002`). That insertion variant is deliberately out of scope: correctly left-aligning a tandem-repeat-expansion indel needs a reference FASTA this project's build environment doesn't have, and — independently — the insertion's own literature-cited rsID (rs869320766, cited by both CPIC's 2018 guideline and PharmVar's own nomenclature paper) could not be resolved against current dbSNP or myvariant.info (checked 2026-08-19). ClinVar cross-references the equivalent variant to a *different* rsID (rs746071566, GRCh38 chr13:48,037,782-48,037,783) — a genuine, citable discrepancy between primary sources, documented rather than silently worked around.

**Practical effect:** this module cannot distinguish `*3.001` (R139C alone) from `*3.002`/`*6` (insertion-carrying) — both present identically at the one position checked here. This does not affect the phenotype conclusion (both are no-function), only allele-*identity* resolution for the rare insertion-carrying case — the same kind of identity-vs-phenotype distinction already established for TPMT's `*3A`/`*3B`/`*3C` handling, just via a different mechanism (deferred locus, not unresolved phase).

**Defining variant** (GRCh38, confirmed directly against dbSNP 2026-08-19; NUDT15 is plus-strand):

| Allele | rsID | Position (chr13) | REF>ALT | HGVS c. | Function |
|---|---|---|---|---|---|
| *3 | rs116855232 | 48,045,719 | C>T | c.415C>T | No function |

**Phenotype evidence:** CPIC's 2025/2026 TPMT/NUDT15 thiopurine guideline update (PMID 41618934, DOI 10.1002/cpt.70209), Table 1 — 0 no-function alleles → Normal Metabolizer; 1 → Intermediate Metabolizer; 2 → Poor Metabolizer, restricted to this module's two-allele scope (the guideline's full table also has "Possible intermediate metabolizer" and "Indeterminate" tiers, both requiring alleles this module doesn't implement). This mapping is unchanged from the 2018 original for the `*1`-vs-`*3` boundary specifically — the 2025 update's genuinely new content (decreased-function tier, possible-IM tier, suballele consolidation) doesn't touch it. NUDT15 uses "Metabolizer" terminology, like TPMT — not SLCO1B1's transport-"function" framing.

**Why there is no unphased ambiguity here (unlike TPMT/SLCO1B1):** those two genes each resolve phase between two linked variants on one haplotype block. NUDT15's in-scope model checks exactly one position — genotype maps to diplotype directly and unambiguously (0/0 → `*1/*1`, 0/1 → `*1/*3`, 1/1 → `*3/*3`). This makes NUDT15 the structurally simplest gene module in this project, a useful RQ2 contrast point.

### Known limitations (deliberate, not oversights)

- **Only 1 of PharmVar's ~20 defined NUDT15 star alleles is recognized** (`*3`, consolidating the former `*2`/`*3`) — the narrowest v1 scope of any gene here, for the real reasons above (no reference FASTA, unresolved rsID discrepancy for the insertion variant).
- **"Possible intermediate metabolizer" and "Indeterminate" phenotype categories can never be produced by this module**, since they depend on alleles it doesn't implement.
- **A confident Tier 2 drug recommendation requires a TPMT result for the same sample** — see below.
- **VCF phase information and multi-allelic ALT fields** have the same limitations documented for TPMT (though this module's single-locus model never has phase to resolve in the first place).

See `pgx_interpreter/genes/nudt15.py`'s module docstring for the full citation trail, including the rs869320766-vs-rs746071566 discrepancy write-up.

**Tier 2 (drug recommendation, added the same session as this gene) — the first two-gene compound recommendation in this project:** via `pgx_interpreter/evidence.py`'s `recommend_compound_thiopurine()`, guideline `PA166104933` (the SAME joint guideline the single-gene TPMT+azathioprine table above cites). CPIC's 2025/2026 update restructured its dosing tables to be keyed on the *joint* TPMT+NUDT15 phenotype rather than either gene alone — there is no "NUDT15 unknown" row, so this function needs both genes' already-computed `PGxResult`s at once and attaches the identical recommendation to both. **All three of CPIC's compound thiopurine tables are now implemented**, selected via `recommend_compound_thiopurine()`'s `drug` parameter (default `"mercaptopurine"`; also `"thioguanine"`, `"azathioprine"` — `pgx_interpreter/cli.py`'s `--thiopurine-drug` flag exposes this as a selector, not an additive flag, since a compound recommendation already attaches to two `PGxResult`s at once). Table 2 (mercaptopurine, malignant AND nonmalignant conditions)'s four rows, restricted to the three phenotype tiers TPMT/NUDT15 can each produce: both Normal Metabolizer → standard starting dose (Strong); exactly one gene Intermediate (either direction) → 30-80% dose reduction (Strong); either gene Poor Metabolizer, regardless of the other gene → 10-fold dose reduction / thrice-weekly dosing for malignancy, or an alternative agent for nonmalignancy (Strong); **both genes Intermediate ("TPMT/NUDT15 compound intermediate metabolizer")** → a deeper 20-50% dose reduction (Strong), CPIC's own new content in this update, reflecting additive toxicity risk. Table 3 (thioguanine, **malignant conditions only**) shares the same four-category shape but genuinely differs in classification strength for the "one Intermediate" and "both Intermediate" rows (Moderate, not Strong — CPIC's own evidence grading is weaker here) and has no separate nonmalignancy branch, since the drug itself is malignant-indication-only. Table 4 (azathioprine, **nonmalignant conditions only**) also shares the shape but, distinctively, has **no reduced-dose fallback at all for the either-Poor-Metabolizer row** — CPIC's text for that row is "Consider an alternative nonthiopurine immunosuppressant therapy," full stop, reflecting a real and more risk-averse clinical stance for nonmalignant indications where a nonthiopurine alternative is more often clinically reasonable. A NUDT15-only report (TPMT not also requested) gets no Tier 2 recommendation at all, correctly — no table in this guideline has a row for that.

See `pgx_interpreter/evidence.py`'s module docstring for the full Table 2 citation (quoted directly) and `pgx_interpreter/cli.py`'s module docstring for exactly when the compound path is taken versus the single-gene TPMT path.

**Validated against real reference material:** 7 real GeT-RM (CDC) consensus-genotype samples (Pratt VM et al. 2022 — the same joint TPMT+NUDT15 GeT-RM paper cited for TPMT above, Table 2). 5 of 7 matched the true consensus phenotype exactly (4 of those also matched at the diplotype level; a fifth, `*1/*5`, presents as this module's `*1/*1` but is still phenotype-concordant since CPIC classifies both as Normal Metabolizer). The remaining 2 samples were deliberately chosen to carry alleles this module cannot see (`*5`, `*4`) specifically to confirm the documented one-locus-scope limitations are real against actual reference data: `NA19079` (real `*2/*5`, true Poor Metabolizer) is undercalled as Intermediate Metabolizer, and `HG01359` (real `*1/*4`, true Indeterminate) is overcalled as a confident Normal Metabolizer. Both are real, documented, and expected — not bugs — see `docs/VALIDATION.md`'s NUDT15 subsection for the full discussion.
