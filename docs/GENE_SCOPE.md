...
**Tier 2 (drug recommendation, added the same session as this gene) — the first two-gene compound recommendation in this project:** mercaptopurine, via `pgx_interpreter/evidence.py`'s `recommend_compound_thiopurine()`, guideline `PA166104933` (the SAME joint guideline the single-gene TPMT+azathioprine table above cites). CPIC's 2025/2026 update restructured its dosing tables to be keyed on the *joint* TPMT+NUDT15 phenotype rather than either gene alone — there is no "NUDT15 unknown" row, so this function needs both genes' already-computed `PGxResult`s at once and attaches the identical recommendation to both. Table 2 (mercaptopurine)'s four rows, restricted to the three phenotype tiers TPMT/NUDT15 can each produce: both Normal Metabolizer → standard starting dose (Strong); exactly one gene Intermediate (either direction) → 30-80% dose reduction (Strong); either gene Poor Metabolizer, regardless of the other gene → 10-fold dose reduction / thrice-weekly dosing for malignancy, or an alternative agent for nonmalignancy (Strong); **both genes Intermediate ("TPMT/NUDT15 compound intermediate metabolizer")** → a deeper 20-50% dose reduction (Strong), CPIC's own new content in this update, reflecting additive toxicity risk. **Scope decision:** only mercaptopurine (Table 2) is implemented for v1 — CPIC's parallel thioguanine (Table 3) and azathioprine (Table 4) compound tables are real, out-of-scope gaps, not silently dropped. A NUDT15-only report (TPMT not also requested) gets no Tier 2 recommendation at all, correctly — no table in this guideline has a row for that.
